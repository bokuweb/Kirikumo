...
| **M5** | Reaching in | Port-forward over WebSocket, one tunnel per local connection, from a chip on the pod (**landed, and verified against a `kind` cluster**: real HTTP through the tunnel, a refusal on the upgrade read as one); exec into a container (Q6); the mount into Ginka's window, **deferred** — the app stands alone for now (§8, 2026-09-12) | Exec |
