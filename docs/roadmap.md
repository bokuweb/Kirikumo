...
What is *not* there yet: log follow, and every write.
...
| **M3** | Pods in depth | Logs *following*, with wrap, find and the previous instance (the container picker and a tail landed in M1); `metrics.k8s.io` for nodes and pods, and owner/child navigation, both landed | Log follow. Metrics in the *table* as well as the panel, which needs a column set that depends on what the cluster can answer |
