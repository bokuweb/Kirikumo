...
- **No exec** yet. Port-forward landed in M5; an interactive terminal into a container needs a terminal emulator, and that is the question §7 Q6 asks.
...
What is *not* there yet: exec (Q6). The mount into Ginka's window is deferred; the app stands alone.
...
| **M5** | Reaching in | Port-forward over WebSocket, one tunnel per local connection, from a chip on the pod (**landed**); exec into a container (Q6); the mount into Ginka's window, **deferred** — the app stands alone for now (§8, 2026-09-12) | The wire protocol has been exercised against the scripted cluster and a local echo, not yet against a live apiserver: the first run against one should confirm the `v4.channel.k8s.io` opening-port frame and that a `403` on the upgrade reads as a refusal |
...
| **Q6** | Exec: an interactive terminal into a container needs a terminal emulator. Ginka has one (`alacritty_terminal`); does this app grow its own, or wait for the mount? | Open. The wire side is the same WebSocket as a port-forward with three more channels; the hard part is drawing a terminal, which is a surface Ginka already owns. With the mount deferred, the answer is either a terminal of our own or a *non-interactive* exec — run a command, show its output — which is most of what a viewer needs from exec anyway. |
