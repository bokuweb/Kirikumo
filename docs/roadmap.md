...
| **M1** | Everything is a table | API discovery, the resource tree in the sidebar, one virtualized table for every kind with `kubectl`'s columns — and a custom resource's own, from its CRD — the namespace picker, health marks, the detail's Overview, Events, YAML and Logs | **Landed.** Sticky header under scroll |
